	...
	return {
		...
		helpers: {
			constructExecutionMetaData: jest
				.fn()
				.mockImplementation(mockConstructExecutionMetaData),
			returnJsonArray: jest.fn((input: IDataObject | IDataObject[]) =>
				Array.isArray(input) ? input.map((json) => ({ json })) : [{ json: input }],
			),
		},
